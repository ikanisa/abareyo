...
const LiveMatchCard = ({ match }: { match: Match }) => {
  ...
  const liveMinute = match.liveMinute ?? (match.events[0] ? `${match.events[0].minute}'` : "LIVE");
    ...
    <GlassCard variant="hero" className="mb-6 overflow-hidden">
      <div className="bg-gradient-hero p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-primary-foreground/90 font-mono text-lg font-bold">{liveMinute}</span>
        </div>

        <div className="flex items-center justify-between">
          <div className="text-center flex-1">
            <div className="text-sm text-primary-foreground/80 mb-2">{match.home}</div>
          ...
          <div className="text-center flex-1">
            <div className="text-sm text-primary-foreground/80 mb-2">{match.away}</div>
            ...
            <div key={`${event.id}-${index}`} className="flex gap-3 items-start">
              ...
              <div className="flex-1 glass-card p-3 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-bold text-sm text-foreground">{event.type.replace(/-/g, " ").toUpperCase()}</div>
...
const UpcomingCard = ({ match }: { match: Match }) => {
  ...
  return (
    <GlassCard className="p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {match.comp ? <Badge variant="outline">{match.comp}</Badge> : null}
      </div>
...
export default function Matches() {
  ...
  const liveMatch = useMemo(
    () => matchesQuery.data?.find((match) => match.status === "live") ?? null,
    [matchesQuery.data],
  );
  const upcomingMatches = useMemo(
    () => matchesQuery.data?.filter((match) => match.status === "upcoming") ?? [],
    [matchesQuery.data],
  );
  const finishedMatches = useMemo(
    () => matchesQuery.data?.filter((match) => match.status === "ft").slice(0, 3) ?? [],
    [matchesQuery.data],
  );
