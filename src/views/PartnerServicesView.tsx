            ...
            <GlassCard className="flex h-full flex-col justify-between gap-5 p-5">
              ...
              <div className="space-y-4">
                <Button
                  className="w-full"
                  variant="hero"
                  onClick={handleStartInsurancePayment}
                ...
                >
                  ...
                  {insuranceSaving
                    ? "Preparing quote…"
                    : `Dial ${formatUssdDisplay(insuranceUssdCode)}`}
                </Button>
                <UssdNetworkTiles ussdCode={formatUssdDisplay(insuranceUssdCode)} />
                <InsuranceStatus status={insuranceStatus} quoteId={insuranceQuoteRecord?.id} />
                {showInsuranceFallback ? (
                  <div className="space-y-3">
                    ...
                    {insuranceQuoteRecord ? (
                      <p className="text-xs text-white/50">Quote ref: {insuranceQuoteRecord.id.slice(0, 8).toUpperCase()}</p>
                    ) : null}
