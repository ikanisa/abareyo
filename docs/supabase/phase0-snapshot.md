...
| Preview secret store | Houses local OIDC/development tokens (legacy managed-host env files were removed). | Confirm they stay scoped to preview; document rotation cadence. |
...
5. Move secrets out of `.env.production`, rotate Supabase keys, and update Vault entries per `docs/supabase/secret-rotation-plan.md` (legacy managed-host copies retired).
