...
import { SupabaseFanAuthService } from './supabase-fan-auth.service';
...
describe('SupabaseFanAuthService', () => {
  ...
  it('throws in production when Supabase credentials are missing', () => {
    ...
    expect(() => new SupabaseFanAuthService(config)).toThrow(
  ...
  it('throws when the service role key matches the publishable key', () => {
    ...
    expect(() => new SupabaseFanAuthService(config)).toThrow(/must not reuse the publishable key/i);
  });

  it('creates a Supabase client when configuration is valid', () => {
    const config = buildConfig();
    const service = new SupabaseFanAuthService(config);
