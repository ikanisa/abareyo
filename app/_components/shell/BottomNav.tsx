...
export default function BottomNav({ localePrefix, activePath }: BottomNavProps = {}) {
  ...
  const navItems = useMemo(() => {
    const consumerNav = appNavigation.consumer.filter((item) => item.surfaces.includes("bottom-nav"));
    return consumerNav.map((item) => {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-black/40 pb-2 shadow-[0_-16px_40px_rgba(0,0,0,0.45)] backdrop-blur-xl"
        ...
        {navItems.map((item) => {
          ...
          return (
            <li key={item.href}>
              <Link
                href={href}
                onClick={() => handleNavSelect(item.title, href)}
                className={clsx(
                  "flex h-12 flex-col items-center justify-center rounded-xl px-2 text-xs font-semibold uppercase tracking-[0.18em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/90 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent",
