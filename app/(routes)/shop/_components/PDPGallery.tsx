...
const PDPGallery = ({ product }: { product: Product }) => {
  ...
  const galleryLabel = t("gallery.section", { product: nameCopy });
  ...
  return (
    <section aria-label={`${galleryLabel.primary} / ${galleryLabel.secondary}`} className="space-y-3">
        ...
        >
          ...
          {scale > 1.05 && (
            <button
              ...
              aria-label={`${t("gallery.reset").primary} / ${t("gallery.reset").secondary}`}
            >
              {t("gallery.reset").primary}
            </button>
          )}
      </motion.div>
    </div>
    <div className="flex gap-2">
      {images.map((image, thumbIndex) => {
        ...
        const thumbnailCopy = t("gallery.thumbnail", { index: thumbIndex + 1 });
        return (
          <button
            key={image.src}
            type="button"
            ...
            aria-label={`${thumbnailCopy.primary} / ${thumbnailCopy.secondary}`}
            aria-pressed={isActive}
          >
            <Image
              src={image.src}
              alt={selectField(image.alt)}
                fill
...
const DEFAULT_FALLBACK = [
  {
    src: "/placeholder.svg",
    alt: {
      en: "Product photography placeholder",
      rw: "Ifoto y'agateganyo y'igicuruzwa",
    },
  },
];
