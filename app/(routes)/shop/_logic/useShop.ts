...
import { SUPPORTED_LOCALES } from "../_data/locales";
    ...
    const searchFiltered = query
      ? base.filter((product) => {
          const localizedSegments = [
            ...SUPPORTED_LOCALES.map((locale) => product.name[locale]),
            ...SUPPORTED_LOCALES.map((locale) => product.description[locale]),
            ...(product.heroCopy ? SUPPORTED_LOCALES.map((locale) => product.heroCopy![locale]) : []),
          ];
          const haystack = `${localizedSegments.join(" ")} ${product.tags?.join(" ") ?? ""}`.toLowerCase();
