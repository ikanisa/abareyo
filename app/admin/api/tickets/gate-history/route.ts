...
export async function GET() {
  try {
    const session = await requireAdminSession([ADMIN_PERMISSION_CODES.GATE_UPDATE]);
    ...
    const response = await fetch(`${backendBase}/admin/tickets/gate/history`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'tickets_gate_history_backend_error', detail: message || null },
        { status: 502 },
      );
    }
    ...
    console.error('Failed to proxy gate history request', error);
    return NextResponse.json({ error: 'tickets_gate_history_proxy_error' }, { status: 500 });
  }
}
