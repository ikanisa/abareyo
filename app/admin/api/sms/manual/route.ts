...
const mapManualRecord = (row: {
  ...
  ingestStatus: 'manual_review' as const,
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      sms: rows.map((row) => mapManualRecord(row as Parameters<typeof mapManualRecord>[0])),
    });
  } catch (error) {
    ...
    console.error('Failed to load manual-review SMS records', error);
    return NextResponse.json({ error: 'sms_manual_fetch_failed' }, { status: 500 });
  }
}
