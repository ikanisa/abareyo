...
import { fetchManualReviewPayments } from '@/services/admin/sms';
...
export const GET = async (request: Request) => {
  ...
  try {
    const payments = await fetchManualReviewPayments(limit);
    return NextResponse.json({ data: payments });
  } catch (error) {
    if (error instanceof AdminServiceClientUnavailableError) {
      return respondWithSupabaseNotConfigured();
    }
    console.error('admin.sms.manual_payments_failed', error);
    return NextResponse.json({ message: 'Failed to load manual-review payments' }, { status: 500 });
  }
};
