...
const PROMO_CODES: Record<string, { percentOff: number; description: LocalizedField }> = {
  FANS10: {
    percentOff: 0.1,
    description: { en: "Members save 10%", rw: "Abanyamuryango bazigama 10%" },
  },
  MATCHDAY5: {
    percentOff: 0.05,
    description: { en: "Matchday offer", rw: "Itangazo ry'umunsi w'umukino" },
  },
  ACADEMY15: {
    percentOff: 0.15,
    description: { en: "Academy bundle", rw: "Igipfunyika cya Academy" },
  },
};
...
const CartContent = () => {
  ...
  const [appliedPromo, setAppliedPromo] = useState<
    { code: string; percentOff: number; description: LocalizedField } | null
  >(null);
  ...
  const appliedPromoCopy = appliedPromo ? translateField(appliedPromo.description) : null;
    ...
    <div className="min-h-screen bg-rs-gradient pb-24 text-white">
          ...
          {items.length === 0 ? (
            ...
            <ul className="space-y-3">
              {items.map((item) => {
                ...
                const productNameCopy = translateField(item.product.name);
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <p className="text-sm font-semibold text-white">
                          {productNameCopy.primary}
                          <span className="block text-[11px] font-normal text-white/60">{productNameCopy.secondary}</span>
                        </p>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          aria-label={`Decrease quantity of ${productNameCopy.primary}`}
                        >
                        ...
                        <button
                          ...
                          aria-label={`Increase quantity of ${productNameCopy.primary}`}
                        >
        ...
        {items.length > 0 && (
          <section className="card space-y-4 bg-white/10">
            ...
            {appliedPromo && appliedPromoCopy && (
              <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-emerald-500/15 px-3 py-2 text-xs text-emerald-100">
                <span>
                  {t("cart.promoApplied").primary} {appliedPromoCopy.primary} ({appliedPromo.code})
                  <span className="block text-[10px] text-emerald-100/80">
                    {t("cart.promoApplied").secondary}
                    <span className="block">{appliedPromoCopy.secondary}</span>
                  </span>
                </span>
                <button
                  type="button"
            ...
            <div className="space-y-2 text-sm">
              ...
              {appliedPromo && appliedPromoCopy && (
                <div className="flex items-center justify-between text-emerald-200">
                  <span>
                    {appliedPromoCopy.primary}
                    <span className="block text-[10px] text-emerald-100/80">{appliedPromoCopy.secondary}</span>
                  </span>
