...
const schema = z.object({
  status: z.enum(["pending", "approved", "denied", "fulfilled"]),
...
export const PATCH = async (request: Request, { params }: { params: { id: string } }) => {
  let session;
  try {
    ({ session } = await assertApiAccess(["admin", "rewards", "analyst"]));
  } catch (error) {
  ...
  const { error } = await client
    .from("reward_claims")
    .update({ status: parse.data.status, processed_at: parse.data.processed_at ?? null })
    .eq("id", params.id);
